    ...
    <>
      ...
      <main className="flex-1 p-4 sm:p-5 overflow-auto page-enter">
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            { label: 'Total Users', value: users.length, icon: Users, accent: '#2563EB' },
            { label: 'Active Users', value: users.filter(u => u.isActive).length, icon: UserCheck, accent: '#059669' },
            { label: 'Inactive Users', value: users.filter(u => !u.isActive).length, icon: UserX, accent: '#EF4444' },
            { label: 'Admin Users', value: users.filter(u => u.role === 'super_admin' || u.role === 'org_admin').length, icon: Shield, accent: '#7C3AED' },
          ].map(stat => (
