      ...
      <main className="page-container page-enter">
        ...
        <div className="card-elevated p-5 mb-6" style={{ background: 'rgba(43,111,224,0.04)' }}>
          <div className="flex items-center gap-2 mb-4">
            <Shield className="w-5 h-5" style={{ color: '#0077D7' }} />
            <h2 className="font-semibold text-sm">National Health System Overview</h2>
          </div>
          <div className="kpi-grid">
            {[
              { label: 'Health Facilities', value: hospitals.length, icon: Building2, color: '#0077D7', bg: 'rgba(43,111,224,0.12)' },
              { label: 'Total Patients', value: totalPop.toLocaleString(), icon: Users, color: '#0077D7', bg: 'rgba(43,111,224,0.12)' },
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          ...
          <div className="card-elevated p-5">
            <div className="flex items-center gap-2 mb-4">
              <Baby className="w-5 h-5" style={{ color: '#0077D7' }} />
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="p-3 rounded-lg" style={{ background: 'rgba(43,111,224,0.08)' }}>
                ...
                <p className="text-xl font-bold" style={{ color: '#0077D7' }}>{birthStats.total}</p>
              </div>
              <div className="p-3 rounded-lg" style={{ background: 'rgba(43,111,224,0.08)' }}>
                <p className="text-xs" style={{ color: 'var(--text-muted)' }}>This Month</p>
                <p className="text-xl font-bold" style={{ color: '#0077D7' }}>{birthStats.thisMonth}</p>
              </div>
            </div>
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                <span style={{ color: 'var(--text-secondary)' }}>Male births</span>
                <span className="font-bold" style={{ color: '#0077D7' }}>{birthStats.byGender.male}</span>
              </div>
              <div className="flex justify-between text-xs">
                  ...
                  {Object.entries(birthStats.byState).sort(([, a], [, b]) => b - a).map(([state, count]) => (
                    <div key={state} className="flex items-center gap-2">
                      ...
                      <div className="flex-1 h-1.5 rounded-full" style={{ background: 'var(--overlay-light)' }}>
                        <div className="h-full rounded-full" style={{ width: `${birthStats.total ? (count / birthStats.total) * 100 : 0}%`, background: '#0077D7' }} />
                      </div>
          ...
          <div className="card-elevated p-5">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              ...
              <div className="p-3 rounded-lg" style={{ background: 'rgba(43,111,224,0.08)' }}>
                ...
                <p className="text-xl font-bold" style={{ color: '#0077D7' }}>{deathStats.total ? Math.round(deathStats.withICD11Code / deathStats.total * 100) : 0}%</p>
              </div>
            </div>
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
        ...
        {assessmentSummary && (
          <div className="card-elevated p-5 mb-6">
            <div className="flex items-center gap-2 mb-4">
              <Activity className="w-5 h-5" style={{ color: '#0077D7' }} />
              <h2 className="font-semibold text-sm">Health System Readiness</h2>
              <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: 'rgba(43,111,224,0.12)', color: '#0077D7' }}>{assessmentSummary.facilitiesAssessed} facilities assessed</span>
            </div>
          ...
          <div className="card-elevated p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="p-3 rounded-lg text-center" style={{ background: 'var(--overlay-subtle)' }}>
                ...
                <p className="text-xl font-bold" style={{ color: '#0077D7' }}>{dqData.totalHISStaff}</p>
              </div>
            </div>
          </div>
        )}
